...
#define NBPFILTER	1
#define M_DEVBUF	0
#define mallocarray(n, s, t, f)	malloc((n) * (s), (t), (f))
#define nitems(a)	__arraycount(a)
#define min(a, b)	uimin((a), (b))
#define caddr_t		void *
#define timeout		callout
#define timeout_set(to, fn, arg) do { \
	callout_init((to), 0); \
	callout_setfunc((to), (fn), (arg)); \
} while (0)
#define timeout_del(to)		callout_stop((to))
#define timeout_add_msec(to, ms)	callout_schedule((to), mstohz(ms))
#define timeout_add_usec(to, us)	callout_schedule((to), \
	MAX(1, (int)(((uint64_t)(us) * hz) / 1000000)))

#define ifq_is_oactive(ifq)	((((struct ifnet *)((char *)(ifq) - \
	offsetof(struct ifnet, if_snd)))->if_flags & IFF_OACTIVE) != 0)
#define ifq_set_oactive(ifq)	(((struct ifnet *)((char *)(ifq) - \
	offsetof(struct ifnet, if_snd)))->if_flags |= IFF_OACTIVE)
#define ifq_clr_oactive(ifq)	(((struct ifnet *)((char *)(ifq) - \
	offsetof(struct ifnet, if_snd)))->if_flags &= ~IFF_OACTIVE)
...
#define MBUF_LIST_INITIALIZER()	{ NULL, NULL }
static inline bool
ml_empty(struct mbuf_list *ml)
{
	return ml->ml_head == NULL;
}
static inline void
...
struct task {
	...
	void (*t_func)(void *);
	void *t_arg;
	volatile unsigned int t_pending;
};

static void
mwx_task_worker(struct work *work, void *cookie __unused)
{
	struct task *task = container_of(work, struct task, t_work);

	atomic_store_relaxed(&task->t_pending, 0);
	task->t_func(task->t_arg);
}

static inline void
task_set(struct task *task, void (*func)(void *), void *arg)
{
	task->t_func = func;
	task->t_arg = arg;
	task->t_pending = 0;
...
static inline void
task_add(struct workqueue *wq, struct task *task)
{
	if (atomic_swap_uint(&task->t_pending, 1) == 0)
		workqueue_enqueue(wq, &task->t_work, NULL);
}

static inline void
task_del(struct workqueue *wq, struct task *task)
{
	if (task->t_pending != 0)
		workqueue_wait(wq, &task->t_work);
	atomic_store_relaxed(&task->t_pending, 0);
}

static inline bool
task_pending(struct task *task)
{
	return task->t_pending != 0;
}

#define PCI_PRODUCT_MEDIATEK_MT7920	0x7920
#define PCI_PRODUCT_MEDIATEK_RZ717	0x0717
...
struct mwx_rx_stats {
	...
	uint64_t	invalid_chan;
	uint64_t	mgmt;
	uint64_t	ctl;
	uint64_t	data;
	uint64_t	data_uc;
	uint64_t	data_mc;
	uint64_t	data_protected;
	uint64_t	beacons;
...
#define MWX_FLAG_CSA_BLOCK_TX		0x08
	...
	uint64_t		sc_intr_tx;
	uint64_t		sc_intr_rx_data;
	uint64_t		sc_intr_rx_mcu;
	uint64_t		sc_mcu_events;
	uint64_t		sc_tx_free_events;
	uint64_t		sc_mcu_timeouts;
	uint64_t		sc_tx_timeouts;
	time_t			sc_last_intr;
	time_t			sc_last_tx_intr;
	time_t			sc_last_rx_intr;
	time_t			sc_last_rx_packet;
	time_t			sc_last_mcu_event;
	time_t			sc_last_tx_free;
...
#define	DEVDEBUG(x)	((x)->sc_ic.ic_ifp->if_flags & IFF_DEBUG)
...
void		mwx_stop(struct ifnet *, int);
void		mwx_watchdog(struct ifnet *);
void		mwx_start(struct ifnet *);
int		mwx_ioctl(struct ifnet *, u_long, caddr_t);

struct ieee80211_node *mwx_node_alloc(struct ieee80211_node_table *);
void		mwx_newassoc(struct ieee80211_node *, int);
...
int		mwx_intr(void *);
void		mwx_softintr(void *);
int		mwx_preinit(struct mwx_softc *);
void		mwx_attach_hook(device_t);
int		mwx_match(device_t, cfdata_t, void *);
void		mwx_attach(device_t, device_t, void *);

void		mwx_reset(struct mwx_softc *);
void		mwx_reset_timeo(void *);
void		mwx_reset_task(void *);
void		mwx_dump_status(struct mwx_softc *, const char *, uint32_t,
		    int);
void		mwx_dump_queue(struct mwx_softc *, const char *,
		    struct mwx_queue *);
...
int		mwx_poll(struct mwx_softc *, uint32_t, uint32_t, uint32_t,
		    int);
int		mwx_init_hardware(struct mwx_softc *);
int		mwx_loadfirmware(const char *, u_char **, size_t *);
...
int		mt7921_mac_fill_rx(struct mwx_softc *, struct mbuf *, int *,
		    uint8_t *);
...
void		 mwx_fill_sta_req_hdr(struct mbuf *, struct mwx_vif *,
...
mwx_init(struct ifnet *ifp)
{
	...
	if (!sc->sc_fw_loaded) {
		rv = mwx_preinit(sc);
		if (rv) {
			printf("%s: init: firmware preinit failed: %d\n",
			    DEVNAME(sc), rv);
			return rv;
		}
	}
	...
	DPRINTF("%s: init\n", DEVNAME(sc));
	rv = mwx_mcu_set_deep_sleep(sc, 0);
	if (rv) {
		printf("%s: init: disable deep sleep failed: %d\n",
		    DEVNAME(sc), rv);
		return rv;
	}
	...
	if (rv) {
		printf("%s: init: enable MAC failed: %d\n", DEVNAME(sc), rv);
		return rv;
	}

	rv = mt7921_mcu_set_channel_domain(sc);
	if (rv) {
		printf("%s: init: set channel domain failed: %d\n",
		    DEVNAME(sc), rv);
		return rv;
	}
	...
	if (rv) {
		printf("%s: init: set rate TX power failed: %d\n",
		    DEVNAME(sc), rv);
		return rv;
	}
	...
	rv = mwx_mac_wtbl_update(sc, mn->wcid);
	if (rv) {
		printf("%s: init: reset WTBL entry failed: %d\n",
		    DEVNAME(sc), rv);
		return rv;
	}
	...
	if (rv) {
		printf("%s: init: add device and BSS failed: %d\n",
		    DEVNAME(sc), rv);
		return rv;
	}
	...
	if (rv) {
		printf("%s: init: set EDCA parameters failed: %d\n",
		    DEVNAME(sc), rv);
		return rv;
	}
	...
	if (ic->ic_opmode == IEEE80211_M_MONITOR) {
		...
		rv = mt7921_mcu_set_sniffer(sc, 0);
		if (rv) {
			printf("%s: init: disable sniffer failed: %d\n",
			    DEVNAME(sc), rv);
			return rv;
		}
...
void
mwx_stop(struct ifnet *ifp, int disable)
{
	struct mwx_softc *sc = ifp->if_softc;
	struct ieee80211com *ic = &sc->sc_ic;
	int s = splnet();
	...
	if (was_running) {
		...
		if (sc->sc_bgscan_arg != NULL)
			free(sc->sc_bgscan_arg, M_DEVBUF);
		...
		/*
		 * Keep WFDMA running across ordinary interface restarts.
		 * Recovery performs a full DMA/WFSYS reset in mwx_preinit().
		 */
	}
...
mwx_watchdog(struct ifnet *ifp)
{
	...
	time_t age, now = getuptime(), oldest = 0;
	int i, txwi_capacity, txwi_used;

	ifp->if_timer = 0;

	txwi_capacity = txwi->mt_count - MT_PACKET_ID_FIRST;
	txwi_used = txwi_capacity - txwi->mt_nfree;
	for (i = MT_PACKET_ID_FIRST; i < txwi->mt_count; i++) {
		if (!txwi->mt_data[i].mt_busy)
			continue;
		if (oldest == 0 ||
		    txwi->mt_data[i].mt_timestamp < oldest)
			oldest = txwi->mt_data[i].mt_timestamp;
	}
	if (txwi_used == 0) {
		sc->sc_tx_timer = 0;
	} else {
		age = oldest != 0 ? now - oldest : MWX_TX_TIMEOUT;
		if (age >= MWX_TX_TIMEOUT) {
			sc->sc_tx_timer = 0;
			printf("%s: device timeout, %d TXWI%s outstanding\n",
			    DEVNAME(sc), txwi_used, txwi_used == 1 ? "" : "s");
			sc->sc_tx_timeouts++;
			if_statinc(ifp, if_oerrors);
			mwx_dump_status(sc, "TX timeout", 0, 0);
			mwx_reset(sc);
			return;
		}
		sc->sc_tx_timer = MAX((int)(MWX_TX_TIMEOUT - age), 1);
		ifp->if_timer = 1;
	}
...
mwx_ioctl(struct ifnet *ifp, u_long cmd, caddr_t data)
{
	...
	case SIOCADDMULTI:
	case SIOCDELMULTI:
		sa = ifreq_getaddr(cmd, (struct ifreq *)data);
...
struct ieee80211_node *
mwx_node_alloc(struct ieee80211_node_table *nt)
{
	/* XXX this is just wrong */
	static uint32_t wcid = 0;
	struct mwx_softc *sc = nt->nt_ic->ic_ifp->if_softc;
	struct mwx_node *mn;
...
void
mwx_newassoc(struct ieee80211_node *ni, int isnew)
{
...
{
	struct mwx_softc *sc = ic->ic_ifp->if_softc;
...
{
	bpf_attach2(sc->sc_ic.ic_ifp, DLT_IEEE802_11_RADIO,
...
mwx_rx(struct mwx_softc *sc, struct mbuf *m, struct mbuf_list *ml)
{
	...
	rv = mt7921_mac_fill_rx(sc, m, &rssi, &chan);
	if (rv != 0) {
	...
	wh = mtod(m, struct ieee80211_frame *);
	if (ic->ic_channels[chan].ic_flags != 0 &&
	    isset(ic->ic_chan_active, chan))
		c = &ic->ic_channels[chan];
	else
		sc->sc_rx_stats.invalid_chan++;

	/*
	 * Hardware scanning changes channels behind net80211's back.
	 * Keep beacon parsing's receive channel in sync with the RX
	 * descriptor, as iwm does for its hardware scan.
	 */
	if (c != NULL && (ic->ic_flags & IEEE80211_F_SCAN) != 0 &&
	...
	sc->sc_rx_stats.last_rssi = rssi;
	sc->sc_rx_stats.last_chan = chan;
	...
	case IEEE80211_FC0_TYPE_DATA:
		...
		if (IEEE80211_IS_MULTICAST(wh->i_addr1))
			sc->sc_rx_stats.data_mc++;
		else
			sc->sc_rx_stats.data_uc++;
		if (wh->i_fc[1] & IEEE80211_FC1_PROTECTED)
			sc->sc_rx_stats.data_protected++;
		break;
	}
	...
	if (__predict_false(sc->sc_drvbpf != NULL)) {
		...
		tap->wr_flags = 0;
		tap->wr_rate = 2;	/* XXX */
		tap->wr_chan_freq = c != NULL ? htole16(c->ic_freq) : 0;
		tap->wr_chan_flags = c != NULL ? htole16(c->ic_flags) : 0;
...
//	mt76_connac_pm_ref(&dev->mphy, &dev->pm);
	...
	if (intr & (MT_INT_TX_DONE_ALL | MT7925_INT_TX_DONE_ALL)) {
		sc->sc_intr_tx++;
		sc->sc_last_tx_intr = getuptime();
		mwx_dma_tx_done(sc);
	}

	if (intr & MT_INT_RX_DONE_WM) {
		sc->sc_intr_rx_mcu++;
		sc->sc_last_rx_intr = getuptime();
...
void
mwx_attach_hook(device_t self)
{
	struct mwx_softc *sc = device_private(self);
	struct ieee80211com *ic = &sc->sc_ic;
	struct ifnet *ifp = ic->ic_ifp;
	...
	/* Override the state machine after ieee80211_ifattach initializes it. */
...
int
mwx_match(device_t parent, cfdata_t match __unused, void *aux)
{
...
void
mwx_attach(device_t parent, device_t self, void *aux)
{
	struct mwx_softc *sc = device_private(self);
	struct ieee80211com *ic = &sc->sc_ic;
	...
	if (pci_mapreg_map(pa, PCI_MAPREG_START, memtype, 0,
	    &sc->sc_st, &sc->sc_memh, NULL, &sc->sc_mems)) {
	...
	sc->sc_soft_ih = softint_establish(SOFTINT_NET, mwx_softintr, sc);
	if (sc->sc_soft_ih == NULL) {
		printf("%s: can't establish soft interrupt\n", DEVNAME(sc));
		goto fail;
	}
	...
	ifp->if_init = mwx_init;
	ifp->if_stop = mwx_stop;
	ifp->if_ioctl = mwx_ioctl;
	ifp->if_start = mwx_start;
	ifp->if_watchdog = mwx_watchdog;
	memcpy(ifp->if_xname, DEVNAME(sc), IFNAMSIZ);

	if (workqueue_create(&sc->sc_nswq, "mwxns", mwx_task_worker, NULL,
...
CFATTACH_DECL_NEW(mwx, sizeof(struct mwx_softc), mwx_match, mwx_attach,
	NULL, NULL);
...
mwx_reset_task(void *arg)
{
	...
	/*
	 * Resets are normally requested after firmware stops responding.
	 * Force mwx_init() through WFSYS reset and firmware reload.
	 */
	sc->sc_fw_loaded = 0;
...
void
mwx_dump_queue(struct mwx_softc *sc, const char *name, struct mwx_queue *q)
{
	if (q->mq_count == 0) {
		printf("%s: ring %s: not allocated\n", DEVNAME(sc), name);
		return;
	}

	printf("%s: ring %s (cons/prod/cpu/dma): %u/%u/%u/%u\n",
	    DEVNAME(sc), name, q->mq_cons, q->mq_prod,
	    mwx_read(sc, q->mq_regbase + MT_DMA_CPU_IDX),
	    mwx_read(sc, q->mq_regbase + MT_DMA_DMA_IDX));
}

void
mwx_dump_status(struct mwx_softc *sc, const char *reason, uint32_t cmd, int seq)
{
	struct mwx_txwi_desc *txwi = &sc->sc_txwi;
	...
	for (i = 1; i < nitems(sc->sc_mcu_wait); i++) {
		if (sc->sc_mcu_wait[i].mcu_cmd != 0)
			wait |= 1U << i;
	}
	txwi_capacity = txwi->mt_count - MT_PACKET_ID_FIRST;
	txwi_used = txwi_capacity - txwi->mt_nfree;
	...
	printf("%s: %s: cmd %08x seq %d state %d flags %x "
	    ...
	    sc->sc_ic.ic_ifp->if_flags, sc->sc_ic.ic_mgt_timer,
	    sc->sc_ic.ic_bmiss_count, txwi_used, txwi_capacity,
	    sc->sc_tx_timer, wait);
	printf("%s: status: fw %08x own %08x dma %08x intr %08x/%08x "
	    "mcu %08x\n", DEVNAME(sc), mwx_read(sc, MT_CONN_ON_MISC),
	    mwx_read(sc, MT_CONN_ON_LPCTL),
	    mwx_read(sc, MT_WFDMA0_GLO_CFG),
	    mwx_read(sc, MT_WFDMA0_HOST_INT_STA),
	    mwx_read(sc, MT_WFDMA0_HOST_INT_ENA),
	    mwx_read(sc, MT_MCU_CMD));
	mwx_dump_queue(sc, "data-tx", &sc->sc_txq);
	mwx_dump_queue(sc, "mcu-tx", &sc->sc_txmcuq);
	mwx_dump_queue(sc, "fwdl-tx", &sc->sc_txfwdlq);
	mwx_dump_queue(sc, "data-rx", &sc->sc_rxq);
	mwx_dump_queue(sc, "mcu-rx", &sc->sc_rxmcuq);
	mwx_dump_queue(sc, "fwdl-rx", &sc->sc_rxfwdlq);
	printf("%s: progress: submitted %llu completed %llu tx-free %llu "
	    "intr %llu tx %llu rx-data %llu rx-mcu %llu mcu-events %llu "
	    "timeouts %llu/%llu\n", DEVNAME(sc),
	    (unsigned long long)sc->sc_tx_stats.submitted,
	    (unsigned long long)sc->sc_tx_stats.completed,
	    (unsigned long long)sc->sc_tx_free_events,
	    (unsigned long long)sc->sc_intr_count,
	    (unsigned long long)sc->sc_intr_tx,
	    (unsigned long long)sc->sc_intr_rx_data,
	    (unsigned long long)sc->sc_intr_rx_mcu,
	    (unsigned long long)sc->sc_mcu_events,
	...
	printf("%s: rx types: normal %llu normal-mcu %llu txs %llu "
	    "unknown %llu parse-errors %llu invalid-channel %llu\n",
	    DEVNAME(sc),
	    ...
	    (unsigned long long)sc->sc_rx_stats.invalid_chan);
	printf("%s: radio rx: mgt %llu beacon %llu probe-resp %llu "
	    "ctl %llu data %llu (uc %llu mc %llu protected %llu) "
	    "hwdec %llu\n", DEVNAME(sc),
	    ...
	    (unsigned long long)sc->sc_rx_stats.data_uc,
	    (unsigned long long)sc->sc_rx_stats.data_mc,
	    (unsigned long long)sc->sc_rx_stats.data_protected,
	...
	printf("%s: activity age: intr %lld tx-intr %lld rx-intr %lld "
	    ...
	    sc->sc_last_tx_intr ?
	    (long long)(now - sc->sc_last_tx_intr) : -1LL,
	    sc->sc_last_rx_intr ?
	    (long long)(now - sc->sc_last_rx_intr) : -1LL,
	    sc->sc_last_rx_packet ?
	    (long long)(now - sc->sc_last_rx_packet) : -1LL,
	    sc->sc_last_mcu_event ?
	    (long long)(now - sc->sc_last_mcu_event) : -1LL,
	    sc->sc_last_tx_free ?
	    (long long)(now - sc->sc_last_tx_free) : -1LL);
	printf("%s: last radio rx: fc0 %02x chan %u rssi %d "
	    "rxd %08x/%08x/%08x\n",
	    DEVNAME(sc), sc->sc_rx_stats.last_fc0,
	    sc->sc_rx_stats.last_chan, sc->sc_rx_stats.last_rssi,
	    sc->sc_rx_stats.last_flags, sc->sc_rx_stats.last_rxd2,
	    sc->sc_rx_stats.last_rxd3);
}
...
{
	char *start = mtod(m, char *);
	char *finish = end;
	int len = m->m_pkthdr.len, adj;

	KASSERT(start <= finish && finish - start <= len);
	adj = len - (finish - start);
	m_adj(m, -adj);
}
...
enqueue:
	...
	while (1) {
		...
		if (rv != EBUSY)
			break;
		q->mq_wakeme = 1;
		tsleep(q, 0, "mwxq", mstohz(100));
	}
	splx(s);
	if (rv != 0) {
		if (cmd != MCU_CMD_FW_SCATTER && seqp != NULL)
			memset(&sc->sc_mcu_wait[seq], 0,
			    sizeof(sc->sc_mcu_wait[0]));
		m_freem(m);
	}
	return rv;
}
...
mwx_mcu_rx_event(struct mwx_softc *sc, struct mbuf *m)
{
	...
	case MCU_EVENT_SCAN_DONE:
		mt7921_mcu_scan_event(sc, m, rxd_eid);
		break;
#if 0
	...
	case MCU_EVENT_BSS_ABSENCE: {
		...
		event = mtod(m, void *);
		if (DEVDEBUG(sc))
			printf("%s: BSS %u %s, free quota %u\n",
			    DEVNAME(sc), event->bss_idx,
			    event->is_absent ? "absent" : "present",
			    event->free_quota);
		if (event->is_absent &&
		    (sc->sc_ic.ic_ifp->if_flags & IFF_RUNNING))
			mwx_dump_status(sc, "BSS absent", 0, 0);
		break;
	}
	case 0x6:
    ...
    uint32_t *val)
{
	...
	s = splnet();
	while (sc->sc_mcu_wait[seq].mcu_m == NULL && rv == 0) {
		rv = tsleep(&sc->sc_mcu_wait[seq], 0, "mwxwait", 3 * hz);
	}
	if (rv != 0) {
		printf("%s: command %x timeout\n", DEVNAME(sc), cmd);
		sc->sc_mcu_timeouts++;
		mwx_dump_status(sc, "MCU timeout", cmd, seq);
		memset(&sc->sc_mcu_wait[seq], 0,
		    sizeof(sc->sc_mcu_wait[seq]));
		splx(s);
		if (sc->sc_ic.ic_ifp->if_flags & IFF_RUNNING)
			mwx_reset(sc);
		return rv;
	}
	...
	memset(&sc->sc_mcu_wait[seq], 0, sizeof(sc->sc_mcu_wait[seq]));
	splx(s);
	return 0;
}
...
int
mwx_loadfirmware(const char *name, u_char **bufp, size_t *lenp)
{
	firmware_handle_t fwh;
	off_t size;
	int error;

	error = firmware_open("if_mwx", name, &fwh);
	if (error != 0)
		return error;
	...
	*lenp = (size_t)size;
	*bufp = firmware_malloc(*lenp);
	if (*bufp == NULL) {
		error = ENOMEM;
		goto out;
	}
	error = firmware_read(fwh, 0, *bufp, *lenp);
	if (error != 0) {
		firmware_free(*bufp, *lenp);
		*bufp = NULL;
	}
out:
	firmware_close(fwh);
...
mwx_load_firmware(struct mwx_softc *sc)
{
	...
	u_char *buf = NULL, *fwbuf = NULL, *dl;
	size_t buflen, fwlen, offset = 0;
	uint32_t n_region, override = 0, option = 0;
	int i, rv, sem;
	...
	if ((rv = mwx_loadfirmware(rompatch, &buf, &buflen)) != 0 ||
	    (rv = mwx_loadfirmware(fw, &fwbuf, &fwlen)) != 0) {
...
out:
	...
	for (i = 0; i < fwhdr->n_region; i++) {
		...
		region = (const struct mwx_fw_region *)((const u_char *)fwhdr -
...
fail:
	if (buf != NULL)
		firmware_free(buf, buflen);
	if (fwbuf != NULL)
		firmware_free(fwbuf, fwlen);
	return rv;
}
...
void
mt7921_mcu_scan_event(struct mwx_softc *sc, struct mbuf *m, uint8_t eid)
{
	struct mt76_connac_hw_scan_done *event;

	if (eid == MCU_EVENT_SCHED_SCAN_DONE)
		return;
	if (m->m_len < 1) {
		printf("%s: scan done event has empty payload\n", DEVNAME(sc));
		return;
	}

	event = mtod(m, struct mt76_connac_hw_scan_done *);
	if ((event->seq_num & 0x7f) != sc->sc_scan_seq_num) {
		DPRINTF("%s: stale scan done: seq %u expected %u payload %d\n",
		    DEVNAME(sc), event->seq_num & 0x7f,
		    sc->sc_scan_seq_num, m->m_len);
		return;
	}
	if ((sc->sc_flags & MWX_FLAG_SCANNING) == 0) {
		DPRINTF("%s: duplicate scan done: seq %u payload %d\n",
		    DEVNAME(sc), event->seq_num & 0x7f, m->m_len);
		return;
	}
...
{
	/* NetBSD's legacy stack supplies only 20 MHz channels. */
...
mt7921_mac_set_timing(struct mwx_softc *sc)
{
	...
	uint32_t slottime = IEEE80211_DUR_DS_SLOT;	/* XXX get from stack */
...
mt7921_mcu_set_tx(struct mwx_softc *sc, struct mwx_vif *mvif)
{
	...
	struct mt7921_mcu_tx {
		...
		.bss_idx = mvif->idx,
		.qos = (ic->ic_flags & IEEE80211_F_WME) != 0 &&
	...
	for (ac = 0; ac < WME_NUM_AC; ac++) {
		const struct wmeParams *q =
		    &ic->ic_wme.wme_chanParams.cap_wmeParams[ac];
		struct edca *e = &req.edca[ac];

		e->aifs = htole16(q->wmep_aifsn);
		e->txop = htole16(q->wmep_txopLimit);
		e->cw_min = htole16((1U << q->wmep_logcwmin) - 1);
		e->cw_max = htole16((1U << q->wmep_logcwmax) - 1);
		e->acm = q->wmep_acm;

		DPRINTF("%s: EDCA ac %d aifs %u cwmin %u cwmax %u "
		    "txop %u acm %u\n", DEVNAME(sc), ac, q->wmep_aifsn,
		    (1U << q->wmep_logcwmin) - 1,
		    (1U << q->wmep_logcwmax) - 1, q->wmep_txopLimit,
		    q->wmep_acm);
	}
...
//	bool insert_ccmp_hdr = false;
	...
	rxd2 = le32toh(rxd[2]);
	rxd3 = le32toh(rxd[3]);
	sc->sc_rx_stats.last_flags = rxd1;
	sc->sc_rx_stats.last_rxd2 = rxd2;
	sc->sc_rx_stats.last_rxd3 = rxd3;
    ...
    struct ieee80211_node *ni, struct mt76_txwi *txp)
{
	...
	if (!multicast && data && ic->ic_fixed_rate != -1)
		fixed_rate = ieee80211_get_rate(ni);
...
mt7921_lmac_mapping(int ac)
{
	static const uint8_t ac_to_lmac[WME_NUM_AC] = {
		[WME_AC_BE] = 1,
		[WME_AC_BK] = 0,
		[WME_AC_VI] = 2,
		[WME_AC_VO] = 3,
    ...
    struct ieee80211_node *ni, struct mt76_txwi *txp)
{
	...
	wh = mtod(m, struct ieee80211_frame *);
	pid = sc->sc_tx_stats.next_pid;
	if (pid < MT_PACKET_ID_FIRST || pid >= MT_PACKET_ID_MASK)
		pid = MT_PACKET_ID_FIRST;
	sc->sc_tx_stats.next_pid = pid + 1;
	...
	ac = WME_AC_BE;
	if (ieee80211_has_qos(wh))
		ac = TID_TO_WME_AC(ieee80211_get_qos(wh) & IEEE80211_QOS_TID);
	...
	/* Protected frames have already been encrypted by net80211. */
#ifdef NOTYET
