...
{
	if (ism->is_iomem)
		ism->vaddr_iomem = (char*)ism->vaddr_iomem + n;
	else
		ism->vaddr = (char*)ism->vaddr + n;
}
...
{
	if (ism->is_iomem)
		memcpy_toio((char*)ism->vaddr_iomem + off, src, len);
	else
		memcpy((char*)ism->vaddr + off, src, len);
}
...
{
	if (ism->is_iomem)
		memset_io((char*)ism->vaddr_iomem + off, c, len);
	else
		memset((char*)ism->vaddr + off, c, len);
}
