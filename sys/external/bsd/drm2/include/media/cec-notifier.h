...
#define cec_notifier_set_phys_addr(x, y)		do { __USE(x); __USE(y); } while(0)
#define cec_notifier_phys_addr_invalidate(x)	do { __USE(x); } while(0)
#define cec_notifier_put(x)			do { __USE(x); } while(0)
#define cec_notifier_get_conn(x, y)		\
	(__USE(x), __USE(y), NULL)
#define cec_fill_conn_info_from_drm(x, y)	do { __USE(x); __USE(y); } while(0)
#define cec_notifier_conn_register(x, y, z)	\
	(__USE(x), __USE(y), __USE(z), (void *)1)
