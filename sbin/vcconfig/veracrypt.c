...
pread_full(int fd, void *buf, size_t len, uint64_t offset)
{
	...
	while (done < len) {
		n = pread(fd, p + done, len - done, (off_t)(offset + done));
		if (n == -1) {
			if (errno == EINTR)
				continue;
			return -1;
    ...
    size_t passphrase_len, uint32_t iterations, uint8_t plain[VC_HEADER_SIZE])
{
	...
	if (passphrase_len > INT_MAX || iterations > INT_MAX ||
	    PKCS5_PBKDF2_HMAC(passphrase, (int)passphrase_len, disk,
	    VC_SALT_SIZE, (int)iterations, EVP_sha512(), sizeof(header_key),
	    header_key) != 1)
		goto out;
	if ((ctx = EVP_CIPHER_CTX_new()) == NULL ||
	    ...
	    VC_ENCRYPTED_HEADER_SIZE) <= 0)
		goto out;
	error = 0;

out:
	EVP_CIPHER_CTX_free(ctx);
	explicit_memset(header_key, 0, sizeof(header_key));
    ...
    char *why, size_t why_size)
{
	...
	if (memcmp(plain + VC_OFF_MAGIC, "VERA", 4) != 0)
		return 1;
	version = be16dec(plain + VC_OFF_VERSION);
	required_version = be16dec(plain + VC_OFF_REQUIRED_VERSION);
	...
	if (be32dec(plain + VC_OFF_HEADER_CRC) !=
	    vc_crc32(plain + VC_OFF_MAGIC,
	    VC_OFF_HEADER_CRC - VC_OFF_MAGIC) ||
	    be32dec(plain + VC_OFF_KEY_CRC) !=
	    vc_crc32(plain + VC_KEY_AREA_OFFSET, VC_KEY_AREA_SIZE)) {
		snprintf(why, why_size, "invalid VeraCrypt header checksum");
		return -1;
	}

	hidden_size = be64dec(plain + VC_OFF_HIDDEN_SIZE);
	volume_size = be64dec(plain + VC_OFF_VOLUME_SIZE);
	data_offset = be64dec(plain + VC_OFF_DATA_OFFSET);
	data_length = be64dec(plain + VC_OFF_DATA_LENGTH);
	flags = be32dec(plain + VC_OFF_FLAGS);
	sector_size = version < 5 ? VC_DATA_UNIT_SIZE :
	    be32dec(plain + VC_OFF_SECTOR_SIZE);
	...
	if (consttime_memequal(plain + VC_KEY_AREA_OFFSET,
	    plain + VC_KEY_AREA_OFFSET + VC_VOLUME_KEY_SIZE / 2,
	    VC_VOLUME_KEY_SIZE / 2)) {
...
out:
	explicit_memset(disk, 0, sizeof(disk));
	explicit_memset(plain, 0, sizeof(plain));
	return error;
}
    ...
    uint8_t volume_key[VC_VOLUME_KEY_SIZE], char *why, size_t why_size)
{
	...
	explicit_memset(volume_key, 0, VC_VOLUME_KEY_SIZE);
